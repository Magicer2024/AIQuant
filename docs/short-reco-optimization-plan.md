...
| **P3-1** | ✅ 已落地（2026-08-10） | 新增 RSI 甜区过滤器 [40,65]：`RSI_SWEET_SPOT` 配置 + `rsi_sweet_spot_series`（与 backtest_enhance/exit 的 rsi14 口径一致）+ `sync.py` 候选 mask 并入（chase 之后）。回测预期胜率 46.9%→47.3%、均值 +0.180%→+0.201% |
| **P3-2** | ✅ 已落地（2026-08-10） | 退出微调：止损 -5%→-6%、止盈 +8%→+10%（TUNABLE_PARAMS 默认值；DB 可覆盖回滚，override 表现无这两项覆盖，改默认值即生效）。回测预期胜率再 +0.4pp（→47.7%）、止损触发率 21%→16% |

实施差异（2026-08-10 校准）：P0-1.1 / P0-1.2 / P0-1.3手段A(v2) / P1-2.3 / P2-3.2 / **P3-1（RSI 甜区）/ P3-2（退出微调 -6%/+10%）** 均已落地；P1-2.2（新鲜度衰减）砍掉、P2-3.1（扩展度排序）与 P0-1.3手段B/C 经回测证伪不实施。详见下方改动总览表与详细项状态标注。
...
| **P3** | 1 | 入场质量：RSI 甜区[40,65] 过滤器（剔除弱势/超买）              | `strategy_params.py`·`rec_filters.py`·`sync.py`     | 低      | ✅ 已落地         |
| **P3** | 2 | 退出微调：止损 -5%→-6%、止盈 +8%→+10%（持仓 3 天不变）         | `strategy_params.py`                                | 低      | ✅ 已落地         |
...
"short_max_hold_days": {                        # 新增（已落地 default=3，2026-08-09 由1→3）
    "default": 3, "type": int,                  # v2 转正后 edge 在 T+3，3 天吃满 T+3 edge
...
```python
HORIZON_MAX_HOLD: dict = {"short": 3, "mid": 60, "long": None}   # short 10→1→3（2026-08-09 由1→3，对应 TUNABLE_PARAMS.short_max_hold_days）
```
...
| 阶段四 | P3-1 RSI 甜区 + P3-2 退出微调（-6%/+10%）                          | ✅ 已落地                  | `RSI_SWEET_SPOT.enabled=False` 降级；`TUNABLE_PARAMS` 改回 -5%/+8% 或写 `strategy_param_override` 覆盖                                 |
...
## 4. 短线胜率/收益增强（2026-08-09 本轮回测验证）

> 背景：v2 引擎 + 3 天持仓 + 止损 -5%/止盈 +8% 已是线上基线（胜率 ~46.9%、均值 +0.180%，见 `backtest_exit.py` 现状行）。  
> 本轮回测目标：在**不碰排序/引擎**前提下，找能同时抬升**胜率与收益**的入场过滤器与退出微调。  
> 配套脚本（纯读取）：`tools/backtest_enhance.py`（入场质量过滤器）、`tools/backtest_exit.py`（真实止盈/止损/持仓退出模拟）。

### 4.1 入场质量过滤器测试结果（在 v2 候选上叠加，多 cohort 聚合）

| 过滤器 | 做法 | T+1 胜率/均值(现状基线 48.6%/+0.100%) | 结论 |
| --- | --- | --- | --- |
| **E1 市场宽度跳过** | 信号日全市场 close>MA20 占比处历史底 20%→跳过 | 48.4%/+0.090%（**胜率与均值双降**） | ❌ **有害，不实施** |
| **E2 缩量回踩** | 信号日 5 日均量 ≤ 10 日均量（健康整理非派发） | 49.2%/+0.105%（+0.6pp 胜率，但 T+5 均值掉到 +0.159%、候选砍半） | ⚠️ 边际，低优先级 |
| **E3 RSI 甜区[40,65]** | RSI(14)∈[40,65]，剔除弱势与超买 | **49.3%/+0.123%**（+0.7pp 胜率、各周期均值全升） | ✅ **唯一稳赚的入场过滤器** |

→ E1（按市场宽度跳过弱市日）直觉合理但数据证伪：弱市日反而是个股回踩的更好买点，跳过反而丢收益。**不要做**。  
→ E3 在所有周期、单独或组合中都稳定贡献 +0.7pp 胜率与更高均值，是唯一值得落地的入场过滤器。

### 4.2 退出规则模拟（真实止盈/止损/持仓，entry=T+1 开盘）

| 配置 | 胜率 | 均值 | 触发止盈% | 触发止损% |
| --- | --- | --- | --- | --- |
| 现状（3天, 5/8, 无E3） | 46.9% | +0.180% | 11.7% | 19.9% |
| **+E3（3天, 5/8）** | **47.3%** | **+0.201%** | 12.3% | 21.2% |
| +E3（3天, 6/10） | **47.7%** | +0.189% | 7.7% | 15.7% |
| +E3（5天, 6/10） | 46.3% | +0.226% | 13.8% | 24.9% |
| +E3（1天, 5/8） | 48.8% | +0.133% | 2.8% | 5.3% |

→ **根本权衡**：延长持仓能抓正漂移（均值 +0.180%→+0.226%），但止损触发率从 ~20% 升到 ~25%、胜率掉到 46%；1 天持仓胜率最高（48.8%）但均值最低。胜率与收益**不可兼得**——延长持仓换收益、压缩持仓换胜率。  
→ 在当前 3 天基线，**加 E3 让胜率与均值同时上升**（46.9%→47.3%、+0.180%→+0.201%）；**再把止损放宽到 -6%/止盈 +10%**（6/10）进一步把胜率抬到 47.7%（止损触发率降到 15.7%），均值基本持平（+0.189%）。这是风险调整后的最优平衡点。

### 4.3 推荐改动（提案，待实施）

| 编号 | 优先级 | 改动 | 落点 | 预期（相对现状 46.9%/+0.180%） |
| --- | --- | --- | --- | --- |
| **P3-1** | P0 | ✅ **已落地（2026-08-10）**：`RSI_SWEET_SPOT={"enabled":True,"lo":40,"hi":65}` 已入 `strategy_params.py`；`rec_filters.py` 已加 `rsi_sweet_spot_series`（口径与 backtest_enhance/exit 的 rsi14 一致，前 13 日 NaN→False 与回测掩码对齐）；`sync.py::_build_signal_records` 候选 mask 已并入（chase 之后、ext 之前），`enabled` 一键降级 | `strategy_params.py`·`rec_filters.py`·`sync.py` | 胜率 +0.4pp（→47.3%）、均值 +12%（→+0.201%） |
| **P3-2** | P0 | ✅ **已落地（2026-08-10）**：`short_stop_loss` 默认 -0.05→-0.06、`short_take_profit` 0.08→0.10（TUNABLE_PARAMS 默认值；DB 可覆盖回滚；override 表现查无此两项覆盖，改默认值即生效） | `strategy_params.py` | 胜率再 +0.4pp（→47.7%）、均值 ≈+0.189%、止损触发率 21%→16% |
| — | ❌ | 市场宽度跳过（E1） | — | 证伪，不实施 |
| — | ⚠️ | 缩量回踩（E2） | — | 边际，低优先级可选 |

**组合落地后预期**：胜率 46.9%→**47.7%**（+0.8pp，+1.7%）、均值 +0.180%→**+0.189%**（+5%），且入场更聚焦（E3 砍掉 ~33% 弱势/超买候选）。

**验证**：`tools/backtest_exit.py` 加 E3 分支已跑通（上表即结果）；**2026-08-10 落地后复跑 `tools/backtest_exit.py`（4373 股全量）复现上表数字完全一致**——组合落地组合（E3, 3天, 6/10）胜率 **47.7%**/+0.189%、止损触发率 15.7%、止盈触发率 7.7%。上线后跑 `tools/diag_short_reco.py` 复验真实推荐 T+1 OC 胜率/均值。
