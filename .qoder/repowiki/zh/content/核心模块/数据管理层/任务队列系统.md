...
## 简介
本文件面向AIQuant任务队列系统，系统采用轻量级内存队列与线程池实现，提供异步任务提交、执行、状态跟踪与查询能力，并通过Flask API对外暴露。同时，系统还包含多Agent流水线编排器、调度器、策略运行器与系统监控模块，形成“任务提交—执行—状态追踪—结果产出”的闭环。

- 任务队列：基于内存字典与线程锁，支持任务提交、状态查询、统计、取消与清理。
- API层：提供任务提交、查询、列表、取消、清理与统计接口。
- 编排与调度：多Agent流水线（数据、信号、回测、风控、报表）与定时调度器。
- 策略运行：策略注册、部署、启停、暂停/恢复与运行统计。
- 监控：系统指标采集与健康状态检查。

## 项目结构
围绕任务队列系统的相关模块分布如下：
- 核心队列：core/task_queue.py
- API路由：routes/tasks.py
- 调度器：scheduler/runner.py、scheduler/state.py
- 多Agent流水线：agents/orchestrator.py、agents/base.py、agents/*.py
...
BASE["agents/base.py"]
DAgent["agents/data_agent.py"]
SAgent["agents/signal_agent.py"]
BAgent["agents/backtest_agent.py"]
RAgent["agents/risk_agent.py"]
...
## 核心组件
- 任务队列TaskQueue：提供任务提交、执行、状态跟踪、统计、取消与清理。
- 任务Task：封装任务元数据、状态、结果与时间戳。
- 任务状态TaskStatus：枚举pending/running/success/failed/cancelled。
- Flask任务API：routes/tasks.py提供提交、查询、列表、取消、清理与统计接口。
- 多Agent流水线编排器：agents/orchestrator.py协调数据、信号、回测、风控、报表Agent。
- 调度器：scheduler/runner.py基于schedule库定时触发同步、扫描与流水线。
...
## 架构总览
任务队列系统采用“API层—任务队列—执行层（线程池）—结果存储（内存）”的轻量架构。Flask API接收任务请求，提交至TaskQueue；TaskQueue通过ThreadPoolExecutor调度执行；任务完成后更新状态并通过回调通知；API层提供查询与统计接口。
...
## 依赖分析
- API层依赖任务队列：routes/tasks.py通过get_task_queue获取单例并调用submit/list_tasks等方法。
...
## 结论
AIQuant任务队列系统以轻量级内存队列为核心，结合Flask API提供任务提交与查询能力；多Agent流水线编排器实现复杂业务流程的有序执行；调度器保障定时任务与流水线的自动化；策略部署与运行器提供策略生命周期管理；系统监控模块提供资源使用情况与健康状态。整体设计简洁高效，适合单机异步任务与数据处理场景。
