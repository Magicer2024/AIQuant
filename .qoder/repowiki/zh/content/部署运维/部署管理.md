...
## 更新摘要
**变更内容**
- 新增DeploymentManager类，提供完整的策略生命周期管理功能
- 新增StrategyRunner类，实现策略运行时管理和状态监控
- 完善部署记录系统，支持策略注册、部署、启停、暂停/恢复、注销
- 增强HTTP路由接口，提供RESTful API支持
- 实现配置热更新和策略内容更新功能
- 添加部署状态监控和故障处理机制
...
## 依赖关系分析
- **routes/deployment.py**依赖**deployment/manager.py**提供的单例管理器。
- **deployment/manager.py**依赖**deployment/runner.py**的运行时类。
- **deployment/manager.py**依赖文件系统进行**deployments.json**的读写。
- **app.py**注册**deployment**蓝图，使HTTP接口可用。
...
- **配置文件**：策略参数与风控规则位于config目录，支持热加载；部署记录位于**deployment/records/deployments.json**。
...
- [config/strategy_params.py:1](file://config/strategy_params.py#L1)
- [deployment/records/deployments.json:1](file://deployment/records/deployments.json#L1)

### 策略函数示例与集成
AIQuant提供了多种策略实现，可以作为DeploymentManager的策略函数示例：

- **综合评分策略**：`strategy_composite()` - 多因子综合评分策略
- **均线趋势策略**：`strategy_ma_trend()` - 均线多头排列策略  
- **MACD策略**：`strategy_macd()` - MACD底背离+金叉策略
- **布林带突破策略**：`strategy_boll_breakout()` - 布林带突破策略
- **多策略融合**：`fuse_signals()` - 多策略加权融合

这些策略函数遵循统一的接口规范，返回包含BUY_SIGNAL/SELL_SIGNAL列的DataFrame，便于部署管理器进行策略执行和信号处理。

**章节来源**
- [strategy/strategy.py:180](file://strategy/strategy.py#L180)
- [strategy/strategy.py:340](file://strategy/strategy.py#L340)
- [strategy/strategies.py:368](file://strategy/strategies.py#L368)