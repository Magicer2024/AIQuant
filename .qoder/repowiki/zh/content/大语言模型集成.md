...
## 更新摘要
**变更内容**
- 全新实现LLM集成模块，包含LLMClient和StrategyAdvisor两大核心组件
- 新增完整的配置系统和路由接口
- 实现智能决策支持、策略建议、市场分析和意图解析功能
- 提供流式对话、健康检查和模型列表等完整API
- 集成本地降级机制，确保系统稳定性
...
## 简介
AIQuant的大语言模型集成模块是一个全新实现的智能决策支持系统，基于LLMClient和StrategyAdvisor两大核心组件，为交易系统提供AI助手功能和自然语言交互能力。该模块支持多种主流大模型（OpenAI、DeepSeek、Moonshot、通义千问等），提供策略建议、市场分析、意图解析、交易理由生成等智能化功能，并具备完善的降级机制和性能优化策略。

## 项目结构
LLM相关能力主要分布在以下模块：
- llm：统一LLM客户端与策略顾问，包含LLMClient和StrategyAdvisor
- routes：LLM对外HTTP接口，提供完整的REST API
- strategy/intent：本地策略意图解析器，作为LLM降级路径
- ministries/libu：报告生成器，支持HTML格式报告
- agents：多Agent流水线编排，与LLM能力集成
...
subgraph "路由层"
ROUTES_LLM["routes/llm.py<br/>LLM 路由接口"]
end
subgraph "LLM核心"
CLIENT["llm/client.py<br/>LLMClient"]
ADVISOR["llm/advisor.py<br/>StrategyAdvisor"]
INIT["llm/__init__.py<br/>模块导出"]
end
subgraph "降级与支撑"
INTENT["strategy/intent/parser.py<br/>本地解析器"]
...
- [llm/__init__.py:7](file://llm/__init__.py#L7)
...
- **流水线编排**：agents/orchestrator.py，将LLM能力融入策略执行链路
...
### LLM客户端（LLMClient）
LLMClient是统一的LLM调用客户端，封装了OpenAI兼容API，提供完整的LLM服务访问能力。

**职责**
- 封装OpenAI兼容API，支持自定义base_url与api_key
- 内置多模型预设（gpt-4o、deepseek-chat、moonshot-v1、qwen-plus等）
- 支持非流式与流式对话，返回统一的LLMResponse对象
- 健康检查：简单测试以验证连接与可用性
...
+quick_chat(user_message, system_prompt) string
+health_check() dict
}
...
StrategyAdvisor是量化策略AI顾问，提供专业的策略分析和市场洞察。

**职责**
- 策略建议：基于个股数据与用户查询，输出结构化建议
- 市场分析：基于市场摘要输出情绪、热点板块、冷门板块、关键事件与操作建议
- 意图解析增强：将自然语言策略描述解析为结构化条件
- 交易理由生成：为订单生成简洁理由说明
...
### 路由接口（routes/llm.py）
routes/llm.py提供了完整的LLM服务REST API接口。
...
本地策略意图解析器提供基于正则表达式的策略解析能力。

**职责**
- 将自然语言策略描述解析为结构化条件配置
- 支持常见策略模式（超跌反弹、放量突破、趋势跟踪、均值回归、动量）
...
- **模块耦合**
  ...
  - llm.client依赖llm.__init__进行模块导出
...
## 故障排查指南
- **常见问题**
  - 未配置API Key或Base URL：检查环境变量与配置文件
...
## 结论
AIQuant的LLM集成模块以"统一客户端+策略顾问+本地降级"为核心架构，实现了全新的智能决策支持系统。通过LLMClient和StrategyAdvisor两大核心组件，系统不仅提供了强大的AI助手功能和自然语言交互能力，还通过完善的错误处理与配置体系保障了稳定性与安全性。配合报告生成与流水线编排，LLM能力可无缝融入策略研发与运营流程。建议在生产环境中严格管理凭据、合理配置参数、持续监控性能与成本，并在合规前提下提供透明的用户提示。