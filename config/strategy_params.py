...
# ── 个股深度 · 推荐跟踪（strategy/deep_tracker.py）────────────────────
# 需求：个股深度板块每日盘后自动推荐后，把候选转成跟踪单，按买点建仓、卖点出场，
#       统计每一笔的持仓时间与盈亏（面板下方「跟踪列表」）。
# 口径：T+1 不判出场、收盘价判定、持仓上限了结与 core/outcome_tracker._evaluate_short
#       保持一致；止盈自 2026-09-08 起改为「本股可达位」固定止盈（见下），不再用移动止盈。
# 关键取舍：**买点必须回踩才成交**（窗口内最低价触及推荐买点才算建仓，涨上去不追），
#       窗口内没回踩就判 expired 且不计入胜率统计 —— 否则"推荐后一路涨"的票会被算成
#       踏空亏损，把胜率统计污染成"推荐质量差"。
# 留空（None）表示跟随同名短线全局参数（short_stop_loss / short_max_hold_days），
#       填值则本模块独立生效。
#
# 止盈口径（2026-09-08 出场多臂实验定案，tools/_eval_deep_exit_arms.py）：
#   深析链的止盈价 = 计划入场价 × (1 + DEEP_TP_AMP_RATIO × 本股平均上涨波幅 up_amp_avg%)，
#   由 stock_deep 在出计划时算好并随 sig_tp 落库，deep_tracker 收盘达价即止盈（固定止盈）。
#   依据（同批交易逐笔配对，往返成本 0.4%，A0=固定止盈2.5rr 持10 为基线）：
#     部署队列 199 票 8648 笔：胜率 68.5%→73.7%，配对均差 +0.388pp (t=+8.49)
#     主板 300 票 12209 笔：  胜率 69.8%→74.3%，配对均差 +0.270pp (t=+5.87)
#     止盈触达率 4.3%→45%，平均持仓 9.7→7.7 天，日均收益 +0.36%→+0.49%
#   旧 2.5rr 止盈中位需涨 +21%，本股常规波幅根本够不着（84.7% 的笔只能到期离场）；
#   k=0.5 均值略高但胜率/触达率/中位全面变差；k=0.33 是胜率与资金效率的平衡点。
#   同实验否决：持5（t=-10~-15 显著变差）、持15/20（尾部与资金效率恶化）、
#   止损cap10%（主板大样本 t=+0.82 不显著）；deep_track 原移动止盈(启动8%/回撤3%)
...
DEEP_TRACK = {
    ...
    # 每日 buy 池挑 top N 时的「强信号档」规则分门槛（2026-09-02 定案，勿凭直觉改）。
    # 背景：buy 池规则分高度集中（score=5 占 88.6%、6 占 10.9%、7 仅 0.6%），
    #   只按 pct_above_ma20 排序（旧逻辑）等于在 88.6% 的低分票里挑贴均线的。
    # 评估（tools/eval_topk_pick.py，46 扫描日 / 3607 只 buy，复刻真实入场出场）：
    #   score=5 均值 -0.05%(PF 0.99) / score=6 +0.62%(PF 1.16) / score=7 -1.18%(PF 0.79)
    #   → 必须**封顶**成两档（>=6 与 <6），不能无脑 score DESC，否则 20 只 7 分极端票主导。
    # 效果：旧键 均值 -0.35%/PF 0.92/胜率 58.0% → 新键 +1.29%/PF 1.37/胜率 62.1%，
    #   逐日 32/46 天胜出，topn 5~20 稳健为正。改动前请先重跑上述评估脚本。
    "top_rank_min_score": 6,
}


# ── 个股深度 EMA20 辅助（2026-08 短线优化，用户提案）─────────────────
# 需求：个股深度买卖建议（strategy/stock_deep.py）以 MA20 为趋势锚（价>MA20>MA60 多头、
#       pct_above_ma20 扩展度守卫、20日区间位置 frac20），用户希望补充 EMA20 辅助短线判断。
# 取舍：MA20 是等权简单均线，对近端价格不敏感；EMA20 指数加权、近端权重更高，能更早反映
#       短线动能拐点。真正的"新增信息"在于 EMA20 相对 MA20 的位置（EMA20 站上 MA20 =
#       近期价格加权强于等权 → 动能增强；跌破 = 动能衰减），而非重复"价相对均线"。
#       因此分数影响设小，避免扰动既有经过数据验证的追高守卫与区间位置守卫。
# enabled 默认 False（基线不变），落地前必须用 tools/_eval_deep_buypoints.py 跑
#   before/after（同口径 T+1/T+3/T+5 胜率与均值），确认增量后再转正。
#   也支持环境变量 DEEP_EMA20_AUX=1/0 临时覆盖 enabled，便于 A/B 验证。
# ⚠ 2026-08 A/B 结论（SAMPLE=120，5376~5524 个 buy/add 信号，同口径 OC）：本配置
#   （价>EMA20 +1 / <EMA20 −1，EMA20>MA20 +1 / <MA20 −1）**明显负贡献**——
#   T+1 61.4%→57.6%（-3.8pp）、T+3 67.3%→61.2%（-6.1pp）、T+5 69.7%→61.9%（-7.8pp）；
#   且买点扩展度均值 +1.26%→+2.92%、20日区间位置≥0.75 占比 22.1%→30.1%（更追高/更高位）。
#   原因：EMA20「顺势加分」系统性抬高了**已上涨、已扩展、区间高位**的买点，恰与 P0-1.3
#   （区间位置守卫，把买点从追涨拉回低吸）方向相反。故维持 enabled=False，不转正。
#   EMA20 仅作面板参考展示（trend.ema20 / pct_above_ema20）与 K 线图数据位。
DEEP_EMA20_AUX = {
    "enabled": False,        # 默认关，验证增量后再转正
    "ema": 20,               # EMA 周期
    "score_above": 1,        # 价站上 EMA20 → +1
    "score_below": -1,       # 价跌破 EMA20 → -1
    "cross_weight": 1,       # EMA20 相对 MA20 动能修正幅（0=关闭）；>阈值才触发
    "cross_band_pct": 3.0,   # EMA20 高/低于 MA20 的触发阈值（%）：仅明显偏离才视为动能增强/走弱
}
...
# ⚠ 2026-08-26 用户决策停用：出场跟踪窗（7-20~8-26）复现该信号线为短线 -37.77%
# 累计收益的绝对主源（56 段 / -53.75% / 均值-0.96% / 胜率41%），挤占了正期望的
# 隔日动量名额（+16.13% / +2.30%）。置 False 后需同步把「缩量回踩」从短线推荐链路
# （short_order_clause 的 priority-1 与三处出口）剔除，见 core/outcome_tracker.py。
PULLBACK_DIP = {
    "enabled": False,
