...
# 口径：出场与 core/outcome_tracker._evaluate_short（方案A）保持一致（T+1 不判出场、
#       收盘价判止损、移动止盈、持仓上限了结），便于与「出场跟踪」面板横向对比。
# 关键取舍：**买点必须回踩才成交**（窗口内最低价触及推荐买点才算建仓，涨上去不追），
#       窗口内没回踩就判 expired 且不计入胜率统计 —— 否则"推荐后一路涨"的票会被算成
#       踏空亏损，把胜率统计污染成"推荐质量差"。
# 留空（None）表示跟随同名短线全局参数（short_stop_loss / short_take_profit /
...
DEEP_TRACK = {
    ...
    "take_profit_pct": None,    # 移动止盈启动线；None → 跟随 short_take_profit
    "trailing_pct": None,       # 移动止盈回撤比例；None → 跟随 short_trailing_pct
